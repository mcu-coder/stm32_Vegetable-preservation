...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagair_conditioner)
    {
        bitFieldBitLen += air_conditioner_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagair_conditioner)
    {
        info->event[info->num] = EVENT_air_conditioner;
        info->num++;
        dataPoints->valueair_conditioner = gizVarlenDecompressionValue(bitFieldOffset,air_conditioner_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += air_conditioner_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtempUpperThreshold)
    {
        info->event[info->num] = EVENT_tempUpperThreshold;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetempUpperThreshold,(uint8_t *)&issuedData[byteFieldOffset],tempUpperThreshold_LEN);
        byteFieldOffset += tempUpperThreshold_LEN;
        dataPoints->valuetempUpperThreshold = gizX2Y(tempUpperThreshold_RATIO,  tempUpperThreshold_ADDITION, elongateP0FormTmp.devStatus.valuetempUpperThreshold); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtempLowerThreshold)
    {
        info->event[info->num] = EVENT_tempLowerThreshold;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetempLowerThreshold,(uint8_t *)&issuedData[byteFieldOffset],tempLowerThreshold_LEN);
        byteFieldOffset += tempLowerThreshold_LEN;
        dataPoints->valuetempLowerThreshold = gizX2Y(tempLowerThreshold_RATIO,  tempLowerThreshold_ADDITION, elongateP0FormTmp.devStatus.valuetempLowerThreshold); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaghumiUpperThreshold)
    {
        info->event[info->num] = EVENT_humiUpperThreshold;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuehumiUpperThreshold,(uint8_t *)&issuedData[byteFieldOffset],humiUpperThreshold_LEN);
        byteFieldOffset += humiUpperThreshold_LEN;
        dataPoints->valuehumiUpperThreshold = gizX2Y(humiUpperThreshold_RATIO,  humiUpperThreshold_ADDITION, elongateP0FormTmp.devStatus.valuehumiUpperThreshold); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaghumiLowerThreshold)
    {
        info->event[info->num] = EVENT_humiLowerThreshold;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuehumiLowerThreshold,(uint8_t *)&issuedData[byteFieldOffset],humiLowerThreshold_LEN);
        byteFieldOffset += humiLowerThreshold_LEN;
        dataPoints->valuehumiLowerThreshold = gizX2Y(humiLowerThreshold_RATIO,  humiLowerThreshold_ADDITION, elongateP0FormTmp.devStatus.valuehumiLowerThreshold); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagCO2Value)
    {
        info->event[info->num] = EVENT_CO2Value;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueCO2Value,(uint8_t *)&issuedData[byteFieldOffset],CO2Value_LEN);
        byteFieldOffset += CO2Value_LEN;
        dataPoints->valueCO2Value = gizX2Y(CO2Value_RATIO,  CO2Value_ADDITION, exchangeBytes(elongateP0FormTmp.devStatus.valueCO2Value));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueair_conditioner != cur->valueair_conditioner)
    {
        GIZWITS_LOG("valueair_conditioner Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagair_conditioner = 1;
        ret = 1;
    }
    ...
    if(last->valuetempUpperThreshold != cur->valuetempUpperThreshold)
    {
        GIZWITS_LOG("valuetempUpperThreshold Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempUpperThreshold = 1;
        ret = 1;
    }
    if(last->valuetempLowerThreshold != cur->valuetempLowerThreshold)
    {
        GIZWITS_LOG("valuetempLowerThreshold Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempLowerThreshold = 1;
        ret = 1;
    }
    if(last->valuehumiUpperThreshold != cur->valuehumiUpperThreshold)
    {
        GIZWITS_LOG("valuehumiUpperThreshold Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaghumiUpperThreshold = 1;
        ret = 1;
    }
    if(last->valuehumiLowerThreshold != cur->valuehumiLowerThreshold)
    {
        GIZWITS_LOG("valuehumiLowerThreshold Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaghumiLowerThreshold = 1;
        ret = 1;
    }
    if(last->valueCO2Value != cur->valueCO2Value)
    {
        GIZWITS_LOG("valueCO2Value Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagCO2Value = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagair_conditioner)
    {
        gizVarlenCompressValue(bitFieldOffset,air_conditioner_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueair_conditioner);
        bitFieldOffset += air_conditioner_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempUpperThreshold)
    {
        devStatusTmp.valuetempUpperThreshold = gizY2X(tempUpperThreshold_RATIO,  tempUpperThreshold_ADDITION, dataPoints->valuetempUpperThreshold); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetempUpperThreshold,tempUpperThreshold_LEN);
        byteFieldOffset += tempUpperThreshold_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempLowerThreshold)
    {
        devStatusTmp.valuetempLowerThreshold = gizY2X(tempLowerThreshold_RATIO,  tempLowerThreshold_ADDITION, dataPoints->valuetempLowerThreshold); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetempLowerThreshold,tempLowerThreshold_LEN);
        byteFieldOffset += tempLowerThreshold_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaghumiUpperThreshold)
    {
        devStatusTmp.valuehumiUpperThreshold = gizY2X(humiUpperThreshold_RATIO,  humiUpperThreshold_ADDITION, dataPoints->valuehumiUpperThreshold); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuehumiUpperThreshold,humiUpperThreshold_LEN);
        byteFieldOffset += humiUpperThreshold_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaghumiLowerThreshold)
    {
        devStatusTmp.valuehumiLowerThreshold = gizY2X(humiLowerThreshold_RATIO,  humiLowerThreshold_ADDITION, dataPoints->valuehumiLowerThreshold); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuehumiLowerThreshold,humiLowerThreshold_LEN);
        byteFieldOffset += humiLowerThreshold_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagCO2Value)
    {
        devStatusTmp.valueCO2Value = exchangeBytes(gizY2X(CO2Value_RATIO,  CO2Value_ADDITION, dataPoints->valueCO2Value)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueCO2Value,CO2Value_LEN);
        byteFieldOffset += CO2Value_LEN;
    }
